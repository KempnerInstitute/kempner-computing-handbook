# Frequently Asked Questions: Data Management and Offboarding
...
- Kempner research is open science where allowed. Prepare shareable data, code,
  ...
  provenance. If data has confidentiality, privacy, or other compliance
  obligations, coordinate with FASRC and the Kempner Institute before placing or
  sharing it on the cluster.
- Use an appropriate long-term repository for data that must be preserved or
  shared, such as Harvard Dataverse
  (https://library.harvard.edu/services-tools/harvard-dataverse) or a funder-
  or discipline-specific repository.
...
- Move anything that must be kept out of scratch and out of Kempner lab/project
...
Data in your FASRC home directory is handled according to FASRC account and
...
## Graduate students and undergraduates working with a PI

What happens to my data after I finish my term at Kempner?

Data in the PI's lab space is managed by the PI or group owner. Before your term
...
## Undergraduates and summer interns using `kempner_undergrad` storage

What should I do before my term ends?
...
## Kempner-affiliated faculty storage requests

My FASRC account is `jharvard_lab` and my Kempner account is
`kempner_jharvard_lab`. How do I increase my storage space?

The Kempner account is intended for compute access. Storage increases should be
requested through the FASRC-associated lab account and storage endpoints, such as
`jharvard_lab`. Contact FASRC for quota or storage expansion requests, and make
sure the requested storage tier matches the lifecycle of the data: persistent
storage for long-term files and scratch only for temporary active-job data.
